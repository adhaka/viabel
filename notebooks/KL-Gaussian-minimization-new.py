...
if divergence == 2:
    ...
    for rho in rhos:
        for d in ds:
            ...
            kl_analytical = gaussianKL(m2, np.diag(np.exp(res.x)), m2, c2)
            inc_kl_analytical = gaussianKL(m2, c2, m2, np.diag(np.exp(res.x)) )
            kl_df = kl_df.append(dict(corr=rho, Dimension=d, IncKL=res.fun, KLMC = inc_kl_val, KLanalytical=kl_analytical,
...
if divergence == 3:
    ds = np.concatenate([np.arange(2,10,2), np.arange(10,75,20,dtype=int)]) # np.arange(2,11,2,dtype=int)
    chi_df = pd.DataFrame(columns=['corr', 'Dimension', 'chidiv' 'KL', 'KLMC',
                                      'IncKL',  'IncKLMC', 'chi','paretok1', 'paretok2'])
